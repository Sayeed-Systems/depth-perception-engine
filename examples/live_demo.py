"""
Depth Perception Engine — live hardware demo.

Pipeline:
    camera (640x240 raw) → split → rectify → disparity → depth → distance → display

Camera is opened directly (not via a StereoCameraStream wrapper) because this
USB stereo camera does not support MJPEG; forcing an MJPEG codec produces
black frames on this hardware.

This is an EXAMPLE, not the library: every perception computation below is
delegated to the installed depth_perception_engine package (stereo/depth/
traversability/obstacles). This script only owns camera I/O, the on-screen
cv2 display, keyboard handling, and desk-test telemetry logging — the four
things the library itself is explicitly not allowed to depend on.

Run:
    pip install -e ..            # installs depth_perception_engine (see pyproject.toml)
...
# ---------------------------------------------------------------------------
# Path setup — allow running `python examples/live_demo.py` directly, so the
# sibling navigation/visualization/telemetry example packages are importable
...
from depth_perception_engine.depth import DepthEstimator, DistanceReader
from depth_perception_engine.obstacles import ThreatAssessor
from depth_perception_engine.stereo import DisparityEngine, RectificationEngine
from depth_perception_engine.traversability import SceneInterpreter
...
CALIBRATION_FILE   = os.path.join(_EXAMPLES_DIR, "config", "stereo_calibration.xml")
...
N_BEAMS            = 20
RECTIFIED_WIDTH    = FRAME_W // 2   # left/right split width after splitting
...
# Traversability grid — partitions each frame for region-level
...
def build_pipeline():
    """Construct and return all pipeline components — every algorithm class
    here comes from the depth_perception_engine library; this function only
    wires them together with this demo's chosen thresholds."""
    from depth_perception_engine.stereo import FrameSplitter
...
def main() -> None:
    run_logger = RunLogger(
        root_dir=os.path.join(_EXAMPLES_DIR, "..", "runs"),
    ...
    try:
        while True:
            ...
            # --- traversability grid (region classification + global decision) ---
            if depth_map is not None:
