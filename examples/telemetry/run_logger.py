...
Example-only: disk-logging of desk-test runs has no place in the
installable library — mp01_perception will have its own ROS-native
diagnostics (see /diagnostics in the mp01_perception ROS package). This
class exists purely for examples/live_demo.py's standalone runs/ folders.
"""
...
SYSTEM_INFO = {
    ...
    "pipeline_stages": [
        ...
        "scene_interpreter (traversability grid) — partitions the frame into a "
...
class RunLogger:
    ...
    def log_frame(
        ...
        pipeline run without the traversability grid enabled) keep working.
        """
        now = time.time()
