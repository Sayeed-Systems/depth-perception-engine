...
**KNOWN DPE V1 LIMITATION (post-freeze, Phase I5.1, accuracy-only — the false-clear safety gap this originally implied was resolved in Phase I6.2/I6.3):** `ClearanceEvidence.nearest_distance_m` can still exhibit 13–94% relative error specifically in sectors whose column overlaps a real depth transition or a narrow obstacle occupying a minority of the sector's own pixels — root-caused to `ThreatAssessor`'s IQR-filter + fixed-percentile column aggregation; interior/non-transition sectors are unaffected (1–6% error, within target). This magnitude limitation is unresolved (no safe accuracy fix found) and remains present. **Safety, resolved**: those same contaminated sectors are now downgraded to `PARTIALLY_SUPPORTED` rather than `SUPPORTED` (occlusion-shadow and wide-ramp reliability gating, threaded into `ThreatAssessor`/`ClearanceEvidence` construction) — 0/252 false-clear sectors in the qualified suite, down from an initial, partly benchmark-methodology-inflated measurement of 28/252. Pure noise still never produces a confidently-`SUPPORTED` sector. See `docs/RELEASE_NOTES_V1.md`'s own "Known caveats" section and `docs/VALIDATION_REPORT.md`'s I1–I6 addendum for the full account.
