...
**Related, re-confirmed at Phase D18 — a cell's `support_fraction` (fraction of its own pixels with valid depth), not `planarity` alone, is the signal that predicts normal-direction reliability.** `benchmarks/i5_surface_opening_clearance/surface/measure.py`, re-run at D18: cells with a real ground-truth plane but partial coverage (`support_fraction` roughly 0.2-0.75 — an object occupying only part of the cell, or straddling a real edge) measured a mean angular error of ~37 deg (individual cells 38-73 deg) even at moderate-to-high `planarity` (0.81-0.89), while full-coverage cells stayed under ~1.5 deg on average (one full-coverage cell measured as high as 81.5 deg under an unfavorable texture/pose combination — the general envelope, not a specific fixture, is wide). This is the same, already-known "high planarity does not guarantee a correct normal on a mixed-content cell" limitation (unweighted-covariance PCA, no robust/outlier fitting, no fit-direction confidence signal independent of planarity) — not a new defect, and not specific to any one downstream integration's own test scene. A consumer working with `SurfaceEvidence` on small or partially-framed objects, or a coarse grid relative to object size, should treat `support_fraction` as a required corroborating check, not just `planarity`.

**RESOLVED (Phase I6.2/I6.3), qualified further at Phase D18 — `ClearanceEvidence` false-clear safety gap.** The post-freeze I1–I6 improvement series (see `docs/VALIDATION_REPORT.md`'s own I1–I6 addendum) initially found `ClearanceEvidence` sectors that reported an obstacle farther/clearer than reality ("false-clear"). That figure was first measured at 28/252 sectors (11.1%), but 24 of those 28 were later found to be an artifact of a benchmark-methodology bug (stale `ThreatAssessor` EMA/debounce state leaking across unrelated fixtures sharing one pipeline instance in the measurement script, not a pipeline defect — fixed in `benchmarks/i5_surface_opening_clearance/clearance/measure.py`). The true, methodology-clean false-clear count was 4/252 (1.6%), traced to two distinct root causes: (1) genuine occlusion-shadow contamination — closed by threading the existing Phase I3 `compute_shadow_zone_mask` reliability signal into `ThreatAssessor.assess()`/`ClearanceEvidence` construction (`PipelineConfig.clearance_shadow_zone_gating_enabled`); (2) a wider (~20px), direction-agnostic StereoSGBM smoothness-regularization ramp that the narrow occlusion-shadow model couldn't see — closed by a second, independent reliability signal, `geometry.reliability.compute_ramp_zone_mask` (`PipelineConfig.clearance_ramp_zone_gating_enabled`). Neither mechanism attempts to *recover* the true near-obstacle value (proven not reliably recoverable once SGBM has smeared a transition — see Phase I6.1's contiguity/nearest-cluster prototypes) — instead, a beam whose IQR-kept population is significantly contaminated by either signal is downgraded from `SUPPORTED` to `PARTIALLY_SUPPORTED` rather than asserted as authoritative. **Result: 0/252 false-clear sectors** in the qualified benchmark, down from the corrected 4/252 baseline, with the worst-case `SUPPORTED`-sector error now 4.4% (down from 139%). The remaining, accepted trade-off: ~30/252 sectors immediately adjacent to a genuine transition (previously `SUPPORTED`, sometimes with a correct reading by chance) now read `PARTIALLY_SUPPORTED` instead — an intentional, conservative-direction cost, not a new defect. Pure decorrelated noise still never produces a confidently-`SUPPORTED` sector (0 of 630 tested).

**"0/252" is the qualified benchmark's own result on its own fixtures, not a universal guarantee — reported directly, per Phase D18.** An external downstream integration (`mp01_perception`, MP01's ROS consumer, a separate repository) independently exercised `ClearanceEvidence` against a different scene (a Gazebo-simulated room, not one of the 252 qualified sectors) and found 3 sectors reporting `SUPPORTED` with a materially larger distance than a real, nearby obstacle in their own bearing range — the same class of contamination Phase I6.2/I6.3 closed for the qualified benchmark's own fixtures, reoccurring on a fixture outside that set. This is exactly the outcome `support_state`'s own three-way split (`SUPPORTED`/`PARTIALLY_SUPPORTED`/`NO_EVIDENCE`) is designed to communicate honestly, not to eliminate: **`ClearanceEvidence` is calibrated, gated evidence about what the stereo pipeline currently supports — not an unconditional clearance guarantee for every possible scene.** A safety-relevant consumer must not treat a `SUPPORTED` sector as proof of clearance on its own; `coverage_fraction`, cross-frame/temporal corroboration, and independent sensing remain the consumer's own responsibility to combine with this evidence, exactly as `docs/PUBLIC_API.md`'s own `ClearanceEvidence`/`GeometryFrame` documentation already frames every other evidence type in this contract (neutral evidence, not a behavioral verdict). No DPE algorithm or threshold was changed in response to this external finding — see the MP01/DPE integration's own `DPE_V1_1_MP01_SIM_MEASUREMENTS.md` for the finding and its own, separate, downstream mitigation (out of scope for and not incorporated into this library).
...
- `1.0.1` — packaging-only patch (Phase D17): a missing `setup.py`
  legacy-build-path fallback caused some non-PEP-517-isolated build paths
  to produce `UNKNOWN` package metadata instead of `depth-perception-engine`.
- `1.1.0` — the post-freeze I1-I6.3 accuracy/safety improvement series
...
- `1.1.1` — **DPE STRUCTURAL CLOSURE.** Packaging-only fix (Phase D18):
  ...
  actual code, no error) under real PEP 517 build isolation on Python
  3.10 — closed by declaring `packages`/`package_dir` explicitly in
  `setup.py` instead. The Box-3 `SurfaceEvidence` angular-error change
  observed downstream (v1.0.1: 20.3 deg -> v1.1.0/v1.1.1: 48.1 deg on one
  external integration's own Gazebo fixture) was root-caused to the
  already-justified, already-applied v1.1.0 SGBM P1/P2 fix shifting which
  pixels are valid in a small, partially-framed object's grid cell — not
  a new defect; re-confirmed as an existing, general "partial-coverage
  cell" limitation on DPE's own offline benchmark (see "Known caveats"
  above) and left unchanged, per this phase's own explicit "do not tune
  against an external fixture" scope. No algorithm, `GeometryFrame`
