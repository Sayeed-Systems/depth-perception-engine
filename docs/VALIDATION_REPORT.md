...
## Phase I1-I6 addendum (post-D17, DPE V1.0.1) — post-freeze improvement series and final qualification

The D-phase series above froze DPE V1 (`0.1.0`→`1.0.0`, packaging-patched to `1.0.1` at D17). A subsequent I1-I6 series (offline, deterministic, no Gazebo/RViz) audited and improved the frozen implementation without any `GeometryFrame` contract change, run against a real "MP01 Gazebo/RViz simulation validation" baseline that was cited in the task prompts but **confirmed, twice independently, not to exist anywhere in this repository or its git history** — `benchmarks/i0_baseline/`'s own README already documents this; the I1-I6 series treated it as an unverified external claim throughout, using `benchmarks/i0_baseline/` and its own newly-built `benchmarks/i1_stereo_accuracy/`, `i3_occlusion_safety/`, `i4_boundary_precision/`, `i5_surface_opening_clearance/`, `i6_temporal/` tooling as the real, reproducible, in-repo evidence base instead.

**Observable stereo ROI, corrected metric (I1/I2):** the previously-reported whole-frame valid-fraction figure (~59.9%) was found to conflate the structural left-edge dead zone (`numDisparities=128` on a 320px frame ⇒ 40% of width is unmatchable regardless of scene content — a `numDisparities`-driven physical limit, not a defect) with genuine implementation loss. Scoped correctly to the actually-observable ROI (columns ≥128), valid fraction measures **~99.3-100%** across every texture/depth combination tested — the pre-existing "≥70% observable-geometry" target was already met once measured correctly; no code change was required to reach it.

**Stereo/disparity accuracy (I1):** `stereo/disparity_engine.py`'s SGBM smoothness-penalty (`P1`/`P2`) channel multiplier was corrected (was using OpenCV's 3-channel figure on grayscale-only input) and `uniquenessRatio` raised 10→20 (`PipelineConfig.block_size` default also revised 13→9) — measured to cut false-valid disparity on fully decorrelated (zero-correspondence) input by ~99% (41.8%→0.27%) with equal-or-better accuracy and no latency cost.

**Occlusion/dis-occlusion local-evidence safety (I3/I4):** a real, reproducible mechanism was found and characterized: a genuine occlusion strip adjacent to a real depth step can read as high-confidence "valid" disparity (StereoSGBM's own smoothness prior extending the confidently-matched near surface across the ambiguous zone — not random noise, numerically indistinguishable from genuine data at the pixel level) and propagate into `obstacle_cloud`/`free_space_rays`/`BoundaryEvidence` while the frame's own `GeometryFrameQuality.overall_state` reads `VALID`. A geometrically-derived (not statistical) shadow-zone reliability signal (`geometry/reliability.py`, Phase I3) is now threaded into the affected Level 3/4 builders, and `geometry/boundary.py`'s own admission logic was recalibrated (Phase I4) to require meaningful fractional support — measured to cut occlusion-strip contamination by ~13.6% and eliminate noise-driven low-support `BoundaryEvidence` false positives entirely (precision/recall 87.8%/100%→100%/100%) with zero regression to genuine (including weak/distant/narrow/occluded) boundaries.

**Opening qualification (I5/I5.1):** `geometry/opening.py`'s span-assembly logic was recalibrated to merge spuriously-split same-depth spans and to treat structurally-unobservable (dead-zone) cells as artificial, non-flank split points rather than rejecting an entire multi-cell span — measured to raise opening recall from 54.5% to **90.9%** with precision remaining 100% and zero regression to width/range accuracy (~0.56% median error) or negative-input false-opening suppression (0 throughout).

**Surface qualification (I5):** high-planarity, well-supported `SurfaceEvidence` normals measured 0.00-1.66° angular error (p95 1.42°) against independently-derived analytic ground truth across fronto/yaw/pitch/combined-slant fixtures at multiple ranges and texture levels — comfortably within the ≤10° target and the ≤5° preferred target.

**KNOWN DPE V1 LIMITATION, accuracy-only (see `docs/RELEASE_NOTES_V1.md`'s own "Known caveats" section for the consumer-facing statement):** `ClearanceEvidence.nearest_distance_m` can exhibit 13-94% relative error in sectors whose column overlaps a real depth transition or a narrow obstacle occupying a minority of the sector's own pixels (root-caused to `ThreatAssessor`'s IQR-filter + fixed-percentile column aggregation, Phase I5.1). Every aggregation alternative that fixed the idealized/synthetic model regressed real pipeline data in a new way; no safe *accuracy* fix was found, and this magnitude limitation remains unresolved and present in V1.

**CORRECTION HISTORY, false-clear safety (I6 Part B, 2026-08-18 → I6.2/I6.3, 2026-08-18, same day):** A first re-audit found the previously-recorded safety statement ("no false-clear sector was observed") inaccurate — a fresh rerun measured 28 of 252 `SUPPORTED` sectors (11.1%) as false-clear (measured range >5cm farther than true). Tracing that number down surfaced two separate findings, both independently verified:
1. **Benchmark-methodology bug**: `clearance/measure.py` reused one `DepthPerceptionPipeline`/`ThreatAssessor` instance across ~9 unrelated synthetic fixtures in sequence; `ThreatAssessor`'s per-beam EMA-smoothing/debounce state (by design, for real video continuity) leaked across those unrelated scenes. Fixed (fresh pipeline per fixture) and reverified directly: 24 of the 28 false-clear sectors were this artifact, not a pipeline defect. True methodology-clean count: 4/252 (1.6%).
2. **Two genuine root causes** for the remaining 4: (a) classical occlusion-shadow contamination (`narrow_obstacle` beam8, 3 sectors) — closed by threading the existing Phase I3 `compute_shadow_zone_mask` signal into `ThreatAssessor.assess()`/`ClearanceEvidence` (`PipelineConfig.clearance_shadow_zone_gating_enabled`, threshold 0.30, chosen with margin below the measured 0.42-0.67 true-positive overlap). (b) A wider (~20px), direction-agnostic StereoSGBM smoothness-regularization ramp (`multi_zone` beam13, 3 sectors) invisible to the occlusion-shadow model even mirrored bidirectionally — closed by a second, independent signal, `geometry.reliability.compute_ramp_zone_mask` (`PipelineConfig.clearance_ramp_zone_gating_enabled`, window_px=24). Neither mechanism recovers the true value (I6.1's contiguity/nearest-cluster prototypes already proved that's not reliably possible once SGBM has smeared a transition) — both downgrade an otherwise-`SUPPORTED` contaminated beam to `PARTIALLY_SUPPORTED`.

**Current, reverified result: 0/252 false-clear sectors** (`benchmarks/i5_surface_opening_clearance/clearance/measure.py`), worst-case `SUPPORTED`-sector error now 4.4% (down from 139% pre-fix). Accepted cost: ~30/252 sectors immediately adjacent to a genuine transition now read `PARTIALLY_SUPPORTED` instead of `SUPPORTED` — conservative-direction, not a new defect. Full regression reverified after both gates: pytest 950/950, `compare_to_baseline` 69/79 exact zero-delta (only latency differs, and improved), I4 boundary precision/recall unchanged at 100%/100%. Pure decorrelated noise still never produces a confidently-`SUPPORTED` sector (0 of 630 tested).

**Temporal qualification (I6):** see this document's own I6 temporal addendum (if present) or `benchmarks/i6_temporal/` for the full quantitative record — static agreement, rotation-compensation improvement, reliability-state correctness, stale-persistence/degradation-recovery/reset/bounded-history behavior, all measured against the existing, unmodified Level-4 stack.

**Final regression, I1-I6 cumulative:** `pytest tests/ -q` — 950 passed (unchanged count from the D17 baseline throughout every I-phase); `benchmarks/i0_baseline/compare_to_baseline` — 69/79 leaf metrics exact zero-delta throughout (only wall-clock `latency_fps.*` metrics vary, as expected); full standalone V1 performance at 320×240 remained within 25-40ms mean / 26-32ms p95 / 27-33ms p99 / 36-40 FPS across every I-phase measurement, comfortably inside the ≤50/≤65/≤80ms, ≥20 (preferred ≥25-30) FPS guardrails throughout.
