...
> **Update (2026-08-06, Phase E3):** built, exactly as anticipated above, with one deviation reported rather than silently resolved: `DepthPerceptionResult.geometry` is typed `Optional[geometry.PointCloud]`, not `Optional[GeometryResult]` — `GeometryResult` was never actually specified (this section itself says "`<GeometryResult>`", a placeholder) and would have bundled `ObstacleCloud`/`FreeSpaceRays`/`GeometryMetrics`, none of which have a producer yet and none of which E3 was in scope to build. `PointCloudBuilder.build()`/`enable_geometry` behave exactly as anticipated — built once per pipeline instance, called from `.process()`, `.process()`'s signature unchanged. `PointCloud` itself is **not** promoted to Tier 1 by this pass (stays reachable via `from depth_perception_engine.geometry import PointCloud`, same Tier 3 path as before) — `DepthPerceptionResult.traversability_mask`'s embedded types were promoted in the public-API-freeze pass because a caller cannot type-hint or compare against them without an import; the same argument could be made for `PointCloud` now that it has a real producer, but promoting it wasn't part of this task's explicit scope, so it's left as a documented option for a future pass rather than decided unilaterally here.
